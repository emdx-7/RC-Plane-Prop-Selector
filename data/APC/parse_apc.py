...
Grids returned (all numpy 2D arrays, same shape):
    ...
    J       : advance ratio V/nD    (dimensionless)
...
# V(0) J(1) Pe(2) Ct(3) Cp(4) PWR-Hp(5) Torque-InLbf(6) Thrust-Lbf(7)
# PWR-W(8) Torque-Nm(9) Thrust-N(10) THR/PWR(11) Mach(12) Reyn(13) FOM(14)
COL = {
    "V": 0,        # mph
    "J": 1,        # advance ratio
    "eff": 2,      # Pe
    "Ct": 3,       # thrust coefficient
    "Cp": 4,       # power coefficient
    "power": 8,    # W
    "torque": 9,   # N-m
    "thrust": 10,  # N
...
if __name__ == "__main__":
    ...
    for p in sorted(glob.glob(os.path.join(here, "inputs", "*.dat"))):
        d = parse_file(p)
